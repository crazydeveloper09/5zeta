...
router.get('/:user_id/two-factor', (req, res) => {
    User.findOne({
        $and: [
            {_id: req.params.user_id},
            {verificationExpires: { $gt: Date.now()}}
        ...
        if(err){
            ...
            if(user){
                let header = "Dwustopniowa weryfikacja | 5zeta.pl"
                res.render("./user/two-factor", {
                    header: header,
                    user: user
                })
            } else {
                req.flash("error", "Kod weryfikacyjny wygasł lub nie ma takiego konta. Kliknij przycisk Wyślij kod ponownie poniżej ")
                let header = "Dwustopniowa werfyikacja | 5zeta.pl"
                res.render("./user/two-factor", {
                    header: header,
                    user_id: req.params.user_id
                })
...
router.post("/:user_id/two-factor", (req, res) => {
    User.findOne({
        $and: [
            {_id: req.params.user_id},
            {verificationExpires: { $gt: Date.now()}}
        ...
        if(err){
            ...
            if(user){
                if(user.verificationCode === req.body.code){
                    ...
                    req.flash("success", `Pomyślnie zalogowałeś się w portalu 5zeta`)
...
router.post("/:user_id/resend/two-factor", (req, res) => {
    User.findById(req.params.user_id, (err, user) => {
        if(err){
            console.log(err)
        } else {
            ...
            async function sendVerificationEmail(user) {
                ...
                const data = {
                    from: `Weryfikacja konta 5zeta.pl <verification@5zeta.pl>`,
                    to: `${Base64.decode(user.email)}`,
                    subject: `Ponowne wysłanie kodu do dwustopniowej weryfikacji`,
                    html: `<html>
                                    <head>
                                        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
                                            integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
                                    ...
                                    <body>
                                        <p class="description">
                                            ...
                                            <br>
                                            Właśnie dostaliśmy prośbę o ponowne wysłanie kodu do
                                            dwustopniowej weryfikacji na portalu ogłoszeniowym 5zeta.
                                            Jeśli to nie byłeś/aś ty, zignoruj wiadomość. 
                                            <br>
                                            Kod potrzebny do weryfikacji to:
                                            <br>
            ...
            sendVerificationEmail(user)
            res.redirect(`/user/${user._id}/two-factor`);
        }
    })
})
